   ...
   private:
    struct Node {
        ...
        Node(const Key& key, const Value& value, const size_t hash, Node* next)
            : keyValue(std::make_pair(key, value)), hash(hash), next(next) {}
    };
   ...
   public:
    class Iterator {
       ...
       public:
        ...
        explicit Iterator(Node* it) : current(it) {}
    ...
    void rehash() {
        ...
        for (Node* node = oldFirstNode; node != _fakeNode; node = node->next) {
            ...
            Node* newNode = new Node(
                node->keyValue.first, node->keyValue.second, newHash,
                _fakeNode
                    ->next);  // создаем новую node с перехешированным ключом

            if (newBuckets[newHash] == nullptr) {
                if (_fakeNode->next != _fakeNode) {
                    newBuckets[_fakeNode->next->hash % _bucketCapacity] =
                        newNode;
                }
        ...
        for (Node* node = oldFirstNode;
             node != _fakeNode;) {	// освобождаем память старых node
            Node* temp = node->next;
            delete node;
            node = temp;
        }
    ...
    std::pair<Iterator, bool> insert(
        const std::pair<const Key, Value>& keyValue) {
        ...
        if (!empty() &&
            temp != nullptr) {	// Проверка, если есть элементы с этим хешем
            temp = temp->next;
            ...
            while (temp != _fakeNode && temp->hash == hash) {
                if (temp->keyValue.first == keyValue.first) {
                    return {Iterator(temp),
                            false};	 // элемент с таким ключом уже лежит
                }
                temp = temp->next;
            }
            temp = _buckets[hash];
            Node* newNode = new Node(keyValue.first, keyValue.second,
                                     hash % _bucketCapacity, temp->next);
            temp->next = newNode;
            ++_size;
            return {Iterator(newNode), true};
        } else {
            Node* newNode = new Node(keyValue.first, keyValue.second,
                                     hash % _bucketCapacity, _fakeNode->next);
            ...
            while (temp != _fakeNode && temp->hash == hash) {
                if (temp->keyValue.first == key) {
                    ...
                    if (temp->next != _fakeNode &&
                        ...
                        _buckets[temp->next->hash % _bucketCapacity] =
                            prevBucket;
                    }
                    if ((prevBucket == _fakeNode ||
                         prevBucket->hash != temp->hash) &&
                        (temp->next == _fakeNode ||
    ...
    float load_factor() const {
        return static_cast<float>(_size) / _bucketCapacity;
    }
};
